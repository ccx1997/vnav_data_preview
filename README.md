...
启动服务后访问 <http://127.0.0.1:5173/#training>，或使用页面顶部的“训练数据”切换按钮。
检查页默认只读扫描 `/mnt/chengchangxu/data/visual_nav_training/`；需要切换产物根目录时，在启动前设置：

```bash
VNAV_TRAINING_ROOT=/训练数据根目录 npm run dev
```

页面支持切换 full/pilot run，按接受、拒绝、子片段和关键词筛选，并以 30 条为一页检查样本。
接受样本按采集预览相同的方位槽位展示六路相机，并展示原始局部占据图、融合后黑白占据图、
静态地图裁剪与 forward route/rollout 叠加、静态障碍补入计数、五步教师动作、同步误差和路线/初态
信息；拒绝样本只展示 Manifest 中的拒绝原因与碰撞诊断，不伪造未物化图片。
页面中的融合 PNG 值域为 `{0,255}`，对应权威教师数组为 `int8 {0,100}`；橙色差异层只属于旧 Pilot
联合预览，不会作为教师输入或本检查页的占据图。

地图路线面板由 case 的 `inputs.npz` 即时派生，不写回训练目录。全局教师地图本体不在每个 case
重复保存；`sample.json.provenance` 记录其 NPZ 路径和 SHA256。每个 case 的 `inputs.npz` 保存当前
`grid_pose` 周围 `400×400 / 20m×20m / 0.05m` 的 `static_occupancy`、同尺寸
`forward_route_mask`、`world_to_pixel` 仿射矩阵、全局 XY 路线点及教师 rollout。

## 数据管理

数据集选择框右侧提供三项本地数据管理操作：

- 地图标签按钮：从 `P_map`、`B9_map`、`B10_map`、`Lift_map` 中选择并确认后，
  为当前数据集 `meta_<数据集ID>/frames.jsonl` 的每条 JSON 记录写入同一个 `map_name`；
  再次确认其他地图会覆盖原标签。写入前会校验所有非空行，存在无效 JSON 时不会修改文件。
...
视频裁剪依赖本机 `ffmpeg`（可通过 `FFMPEG_PATH` 指定可执行文件）。标签覆盖、裁剪和删除
均应在确认当前数据集与参数无误后执行。
