...
Uses observed XY/yaw only, no teacher output or source owner. A bounded evidence
window confirms a directional lobe; complete saved windows provide retrospectively
estimated boundaries. Confirmation latency/live recorder equivalence is untested.
"""
...
def geometry(p):
    ...
    signed = math.degrees(math.atan2(float(np.cross(direction_a, direction_b)), float(direction_a @ direction_b)))
...
def classify_lobe(p, cfg):
    ...
    if bounded_angle >= cfg.turn_angle_deg and geom['max_translation_m'] <= .10:
        ...
        branch = 'turn'
    elif (bounded_angle >= cfg.detour_angle_deg and geom['path_m'] >= .3 and aligned
        ...
        branch = 'detour'
          ...
          and consistency >= .85 and aligned and abs(heading) >= max(2., total_angle * .4)
