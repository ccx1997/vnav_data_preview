...
## 2026-09-24 新版灰线地图坐标映射

- 结合会话 `01a0d2c1-2f2e-7971-bdc7-9a4610bc7196`：灰线是道路，人工断口表示小车不可通行，
  矩形建筑保留为 RGB 匹配背景。用户确认 `visual_navigation_e2e/data/source_map.png` 沿用旧图
  放大后的位置关系，因此复用现有世界→P_map 和配对道路弧长关系；新图不是任意图片自动配准。
- 新开关 `export_pixels.py --handdraw-map source-map`，默认 `legacy` 不变。旧 JPG 中心
  `(u,v)` 先按 `((u+.5)*2192/1441-.5,(v+.5)*1642/1079-.5)` 转新图，再逆时针 90°。
  输出 **1642×2192**，只旋转、不插值，保留 0/192/255 三色和三处断口。注册资产已纳入仓库并校验哈希。
- 新模式关闭 4 m/1.5 m/1 m 的全部端点吸附，保留连续道路进度。道路仅认灰色 192；局部栅格角点
  允许最多 2 px 校正。已配对主环路/支路 1,313 个弧长采样点全部落在新灰线，三个断口的跨越检查
  均拒绝。新画的未配对道路仍缺少世界坐标，不通过最近灰线猜配；不把名义米/像素当全图精确比例。
- 用户随后明确授权对现有数据重新做**坐标映射**，并允许截掉/丢弃进楼样本。执行规则：当前点或
  任一历史点不在灰线则丢弃整样本；路线起点无效也丢弃；路线在首个非灰线点/跨非灰线线段前截断，
  不跳过中间段再接回。相邻点按每轴 ≤0.5 px 步长检查；全局像素 `route` 与 `handdraw_route_pixel_xy`
  保留同一前缀。历史及其时间戳完整保留，yaw 和教师命令原样保留；不重新生成/验证动作标签。
- 新版标记 `vnav_pixels_source_map_v2`，`map_scope=["P_map"]`，`handdraw_map=source_map`。
  用户明确本次简笔地图仅对应 P_map；**新版模式不得混入 B10 等其他地图**。CLI 在坐标转换前按
  `map_not_supported_by_source_map` 排除非 P_map 行，直接调用转换器则报错，旧 `legacy` 多地图模式不变。
  输出有效性、路线截断计数、`excluded_rows.jsonl`、灰线 mask、schema 和哈希审计。
  源 frames.jsonl、人工 map_name/视频裁剪、教师数据和旧像素版本全部保留；不重新下载/解包/运行教师。
- 批量入口 `processing/coordinate-converter/remap_existing.py` 显式选择每任务最新成功 `full` run，
  不含 pilot 或重复旧 run。当前发现 **18 个任务**；新版导出必须是新的独立目录。逐任务检查所有
  输出点/线段落在灰线、保留＋剔除数闭合、坐标往返和源/输出 SHA-256；源不完整时只报告。
- 完成验证：坐标转换与原数据流水线 **65/65** 回归通过，覆盖新旧模式、灰度/断口保持、样本剔除、
  路线截断、混合地图、重复坐标缓存逐值等价及源数据保护。新模式缓存不量化坐标，批处理使用 4 个
  独立 CPU 作业；未使用 GPU 或重新执行教师推理。
- 首次扫描 18 个任务、15,980 条 accepted 时误将 1,184 条 B10 原地图记录一起保留；它们并未套用
  简笔图，但超出了用户的 P_map 范围。当前有效数据集只含 **14,796** 条 P_map 输入，保留
  **10,245** 条，剔除 **4,551** 条（当前点不在灰线 **4,419**、历史不在灰线 **132**），其中
  **5,785** 条保留路线被截断。13 个任务含 P_map 输入，其中一个任务的 P_map 样本全部被剔除；
  5 个只有其他地图的任务不进入本次数据索引。
- 全量核验 **526,256** 个保留 P_map 当前/历史/路线点、**446,973** 个相邻路线线段均在灰线，
  世界坐标往返最大误差 **4.2633e-14 m**，yaw/历史时间/命令/初态/RGB 引用保持；**452** 份源
  frames/视频清单的大小、mtime、SHA-256 前后一致。**32,003** 个教师/地图/资产文件在任务结束和
  整批结束时 SHA-256 复核一致，输出文件哈希全部通过。
- **当前使用的数据**：`/mnt/chengchangxu/data/visual_nav_training/_source_map_exports/20260924_pmap_only/`，
  `summary.json` 提供纯 P_map manifest/schema 路径。范围纠正复用已有像素结果，保留记录逐字节一致，
  112 个读取的上游导出文件前后 SHA-256 一致；65/65 回归通过（包含无 B10 地图配置时直接跳过、
  直接接口拒绝非 P_map、纯非 P_map 输入产生空 manifest 及旧模式多地图保护）。
  之前混合版 `20260924_source_map` 仅留作历史审计，不再作为本次数据入口；未重算坐标或覆盖任何源数据。
  过滤后的记录不能假定时间连续，后续序列训练需依据源时钟/缺口重新分段；本次不改源 route_id。
  [新版入口说明](processing/coordinate-converter/README.md)、
  [当前 P_map 专用报告](reports/source_map_20260924/pmap_only_report.md)、
  [首次导出历史报告与覆盖图](reports/source_map_20260924/report.md)。

2026-09-25 只读核对当前 `20260924_pmap_only/summary.json`：实际提供保留样本的任务为以下
**12 个**，合计 **10,245 条**。本次仅补充索引说明，没有重新处理数据。

| 数据任务 | 保留 P_map 样本数 |
| --- | ---: |
| `20260818193938Axg` | 1,483 |
| `20260819203318qFl` | 1,038 |
| `20260820180215WDK` | 1,356 |
| `20260821114524n4Q` | 150 |
| `202608251919001QS` | 505 |
| `20260828154654XXz` | 650 |
| `20260902153110uGf` | 804 |
| `20260903154146O1X` | 1,005 |
| `20260917174539CCx` | 390 |
| `20260918200506GCy` | 1,200 |
| `20260920122356WIt` | 800 |
| `202609201240128Up` | 864 |

`20260831152735OM4` 的 50 条 P_map 输入全部被筛除，索引保留空结果作审计，不贡献有效样本。
另有 `20260824161657Li2`、`20260826154539RI5`、`20260827180935lmd`、`2026083119533604q`、
`20260901185939a1N` 因无 P_map 输入而跳过。
