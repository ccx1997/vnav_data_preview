...
- 复现同事反馈：6 s 延迟下，1–8 s 空洞后的未来连续段可令 `start > decision`，随后 end
  抛异常。现将该候选延后至 decision 到达新连续段；Turn/Straight 的 early end 及 Turn 的
  early EOF 均无副作用返回 `(False, ts)`，不改变活动状态/扫描进度。未放宽转弯确认阈值。
- 普通 `gap >0.5 s` 仍切开几何证据，禁止跨洞插值、平滑或累计转角；撤销 09-21 将所有 gap
  强制切片的规则，允许已确认的转弯录制窗口保留定位未知的上下文。直行恢复/稳定停车必须
  重新积累连续证据。位置 >2 m / yaw >45° 的跨洞变化仍保守截断，45 s 上限与实际 EOF 保留。
  缺失定位的媒体不能直接当作有效监督，训练侧现有 pose 有效性与断段过滤不变。
- 接入示例改为同域单调媒体时间减 6 s，避免 `pose_cache[-1]-6` 在断流时停表；保留已有
  60 s pose 历史缓存，活动窗口仍须接入 `should_save`、Tmax 和 EOF 排空。
- 67/67 单测、68/68 延迟空洞回放（无洞对照及 1–8 s 单/双洞 × 两种时钟 × 两相位）、54/54
  既有几何流式回归通过；专家独立复核新增 7 项契约测试通过。旧版空洞回放仅无洞 4 组通过，
  有 10 次起点晚于 decision、40 次 early end 异常；修复后均为 0。两个 1 s 洞的持续弯由 3 段
  变为 1 段，没有跨洞推断未观测轨迹。
- 复用原 159 段源数据回放：仍有 84 个源段可保存，窗口由 87 降至 84；7 个源段窗口改变，
  其中 `0902-auto2` 1/8/13 合并，另 4 段延长上下文。32/32 明确转弯保留，69/69 明确近直线
  不启动，旧盲评 29/29 一致，19/20 仍排除。这些是开发回归结果，不是线上总体 recall。
- 640 个源文件与前次审计的大小/mtime/SHA-256 一致；CCx_50 元数据不一致仍仅报告并排除。
  未重新下载/解包/处理源数据，未改 `frames.jsonl`、人工 `map_name` 或裁剪，未部署采集系统。
  [复现、修复与验证报告](reports/gate_delayed_gap_20260922/report.md)，测试状态按项目约定同步飞书。

## 2026-09-21 独立随机盲评后优化生产 Gate

- 新增独立标注员 subagent；固定 seed `2026092103` 从 159 个有效片段随机抽取 24 个，再加
  5 个不重复重点对照，打乱并隐藏来源/算法/历史标签。随机组 12 转弯、12 直线；对照组
  1 转弯、4 直线。6 个浅弧边界为中置信度。离线修正规则先达到 29/29 盲评一致，之后才改代码。
- 生产 `data_collection_gate.py` 已由局部曲率触发改为有界持续转向确认：2° 回转死区；常规
  yaw≥8° + 路径≥0.15 m + 同向 XY 方向变化≥3°；短慢弯 yaw≥4°、0.08–0.80 m、主要转角
  形成≥1.5 s、几何一致，必须波段闭合或极值后稳定≥0.5 s。原始证据和平滑同限连续15 s；
  6 s 调度延迟下可有界回看。弱证据返回 uncertain，不自动成为正式转弯样本。
- 保存核心/上下文分离：默认前置4 s，确认恢复直行后再保留4 s，通常带约5–6 s驶离画面。
  退出改用稳健整窗直线残差≤3 cm、yaw极差≤3°、净位移/路径≥0.95，并检查4 s持续转向趋势，
  同时处理“2 cm摆动拖至45 s”和“缓慢宽弧过早结束”问题。停车3 s、定位jump/gap、45 s上限
  和实际媒体末端优先；长弯可凭跨切点的确认核心续段，默认重叠4 s。新增EOF结束接口。
- 所有结束均验收最终目标，提前截断只用实际保存窗复核；spin平滑支持纳入观测终点，修复EOF
  借用后续旋转误保留的反例。仍保留原2 s spin契约，不能承诺更慢原地旋转的召回。
- 最终159段按6 s延迟/60 s缓存/0.2 s节拍逐段回放：84个源段产生87个拟保存窗，中位11.64 s；
  同协议旧版141个源段有可保存输出。此前32个明确几何正例全保留、69个明确负例全部不启动
  （旧版负例61启动/51保存），新盲评29/29一致；原23个模糊例20保留，不把它们计作正确标签。
  19/20及GCy42均排除。样本用于开发选择，且只有已采片段，不是线上总体precision/recall估计。
- 验证：60/60单测、54/54生产流式已知几何检查（27场景×2相位）、4项专家独立定向复核通过；
  编译与diff检查通过。旧启停时间用显式无额外上下文参数保留验证，默认上下文另测。
  60 s/20 Hz输入的直线启动检查中位耗时由8.71增至20.41 ms，低于所测200 ms周期，但非线上性能保证。
- 640个源文件大小/mtime/SHA-256未变；继续排除CCx_50元数据不一致问题。未下载、解包、覆盖
  frames.jsonl/map_name/裁剪、应用拟采窗口或重跑教师。已有文件缺失上下文无法补回；真实录制器
  需要媒体缓存、should_save、续段和EOF排空接入。本次仅修改仓库，未部署远端。
- [完整报告](reports/gate_method_revision_20260921/report.md)、
  [盲标](reports/gate_method_revision_20260921/blind_labels.json)、
  [最终逐段回放](reports/gate_method_revision_20260921/current_replay.json)、
  [接入说明](readme_gate.md)。基线、盲评、迭代测试及最终结果按项目要求同步飞书。

## 2026-09-21 已采数据上的转弯原型离线试验

- 按用户要求复用 8Up/WIt/GCy/CCx 四个已有任务：160 个本地目录中 159 段进入对照，合计
  7,712 个有效 pose。CCx_50 的 Meta keep 窗 5 s 与视频 Manifest 约 7.589 s 不一致，跳过并
  记录，未恢复/刷新。640 个读取源文件大小、mtime、SHA-256 前后一致；不改原始数据、地图、
  裁剪、教师或生产 Gate。实验脚本、标签、拟采集窗口及图均在独立报告目录。
- 初版用去噪方向波段、路线方向拟合、转向/弱绕行/短慢弯/原地分支。8Up 校准横移门槛由
  0.15 m 调至 0.20 m 后，WIt 留出检查仍误收 14/15/18/19 四个近直线片段，保留失败证据。
  原因是小方向估计偏差经长路程放大，且几何区间尚未与最多 15 s 的角度证据统一。
- 保守消融将仅弱绕行证据送复核档；常规方向阈值 8°、短弧弱分支 4°、小反转死区 2°，
  前后上下文各 4 s，核心间隔 3 s，45 s 分段上限/2 s 重叠、最短保存 4 s。
  修复了超过 180° 的路线方向折返误判。该原型为离线回溯，不是已验证的线上状态机。
- 全部 159 段输出 **85 确认、32 待复核、42 直行**；当前 Gate 仅在保存窗口内回放有保留
  窗口的源片段为 155，二者均不能等同于完整生产采集表现。未生成/覆盖源视频或应用筛选。
- 数据专家在未知对应输出时独立审阅前三批 124 段的原始 XY/yaw 图：32 个明确几何正例、
  69 个近直线、23 个不确定，非人工视频金标。保守版明确正例 32/32 确认；近直线 1 确认、
  27 待复核、41 直行；边界例 20 确认、3 待复核。8Up 19/20 均判直行，多个 S 形路线保留。
  WIt 参与了保守版选择，不能继续称独立留出；冻结后新检查 GCy 保留 19/19 明确正例，
  22 个近直线中误收 1 个（42：约 0.8 s、5.8°、局部侧移 2.8 cm 的短促修正），未再据此调参。
- 27/27 离线已知几何检查通过，含17个历史低速/稀疏/停走/短弧场景及10个新增正负/边界场景；
  不能替代原42项API测试、因果延迟验证或生产recall评估。当前原地分支实验允许15s角度累积，
  不等于已部署的2s原地判据。
- 拟保留88个窗口，中位时长11.58 s；81个前置不足4s、86个后置不足4s，0个双侧均满4s。
  已有源窗口/定位断点/停车硬停限制了可用上下文，没有外推或拼接缺失区间。前后各4s的完整
  效果需未来录制器真实媒体缓存支持；本次不能从已有裁剪数据补回画面。
- [详细报告与例图](reports/gate_offline_trial_20260921/report.md)、
  [逐段结果](reports/gate_offline_trial_20260921/conservative_trial.json)、
  [独立几何标签](reports/gate_offline_trial_20260921/review_labels.json)、
  [定向回归](reports/gate_offline_trial_20260921/regression.json)。回归阶段与最终结论均已同步飞书。

## 2026-09-21 转弯采集 precision/recall 与场景上下文方案评审

- 按用户要求咨询一个数据专家 subagent，目标改为识别有意义的拐弯/绕行，小幅局部摆动视为直行，
  同时保留接近及驶离过程。推荐“宽松候选 → 稳健动作核心确认 → 上下文包装 → 最终窗口验收”。
  本轮只做设计评审，未改 Gate/录制器/训练处理，未运行新算法实验或重处理任何源数据。
- 推荐用去噪后的同向转动波段与较长空间范围的路线变化互补确认，保留独立原地转向；
  相邻三点曲率降为辅证。不能使用首尾净 yaw 筛 S 弯，也不能直接累加逐帧绝对 yaw。
- 待标定起点：候选约 3–4°；常规转向约 8–10°；绕行约 0.15–0.25 m 横向变化加可靠路线方向
  变化；短慢弯保留约 4–6° 加可信几何支持的独立分支。上述不是生产阈值或已验证效果。
  历史 0.16 m 短弧总角仅约 5.5°，统一增大 yaw/位移门槛会损伤旧召回；模糊候选单独复核，
  不将候选缓存 recall 冒充正式样本 recall。目标语义变化须显式记录，不能静默删除旧正例。
- 首版建议以真实核心边界**前后各扩展 4 s**，分别记录 core/capture 时间。允许合并接近的
  绕行动作，保留子核心；现有停车、定位跳变和 45 s 上限优先，长事件连续分段。
  不能直接前移 `active_start_timestamp_s`，避免上下文停车触发核心之前的硬停止；必须确认
  录制器具备足够媒体历史，pose 缓存本身不足以补回视频。
- 自然结束也需验收最终窗口内的确认核心，不能用延长片段将误触发凑成合格样本。验证分开统计
  事件 precision/recall、核心时间覆盖及前后上下文，不能把期望保留的直行上下文算误采。
  必须用完整连续流评估漏采，并复用 0917 召回与 19/20 真实误触发反例。
- 已核对训练端仍要求 2 m 前视路线，并有静止首尾裁剪（过渡上下文 1 s）；采集前后 4 s 不等于
  低速样本已有充分训练前视，实施时需一并审计核心在训练输出中的覆盖。本轮保持原参数。
- [完整方案、参数候选、集成边界和专家结论](reports/gate_0902_auto2_20260921/proposed_design.md)。

## 2026-09-21 0902-auto2 近直线 4 秒片段只读诊断

- 标题 `0902-auto2` 实际对应 2026-09-20 的 `202609201240128Up`。只读复用本地 33 个
  片段，不下载、解包、裁剪、重新导出或重跑教师，不修改 Gate 或人工 `map_name`。
- 19/20 的原始 owner 均为 `turn`，采集/导出/视频 Manifest 窗口一致，精确时长为
  `4.271232/4.189357 s`。保存 pose 路程 `2.266577/3.896869 m`，相对首尾连线最大偏离
  `2.081/2.110 cm`，yaw 极差 `2.406/3.048°`；短时长在采集窗口中已经存在。
- 当前默认 Gate 直接读取两例保存 pose，在开始后 1 s 的锚点得到三点曲率中位数
  `10.752/12.682°/m`，超过 `8°/m`，两例均复现 `start_collection=True`；弧线分支
  不要求明显 yaw 变化。证据所需平滑和路线均位于保存范围内，绝对/相对时间结果一致。
  9 月 18 日修正前版本 `048c0f6` 对同样输入均不启动；真实顶点抽稀改善稀疏弯道召回的同时，
  暴露了小幅摆动误触发问题，不能直接回退而忽略既有召回。
- 自然结束后缺少独立整段转弯质量验收；`>=4 s` 只筛时长。当前启停规则可以使局部误触发
  几秒后结束，保留略高于下限的近直线片段；不是固定 4 秒切片。线上具体结束原因/部署版本
  缺少完整高频缓存、实际参数及 reason 日志，尚不能精确还原。
- 33 段含 32 turn/1 straight，其中 11 段为 `[4,5) s`，全部是 turn，且均符合本次描述性
  粗筛 `yaw 极差<8° AND 路程/首尾距离−1<0.002`；全批 22 段符合该粗筛。
  该统计不是生产过滤规则、人工真值或整体误采率，未用于改写源/教师数据。
- 验证：33 段窗口与有效递增位姿、两个真实例子新旧启动对照、10 路 MP4 时长核对通过；
  142 个读取源文件的大小/mtime/SHA-256 前后一致。19 的 cam1/cam6 原有覆盖缺口为
  1.335/0.076 s，单独记录，未刷新源数据。后续应评估抗扰曲率证据与保存质量验收，并保持
  已有低速、停走、稀疏与原地转弯召回；本轮仅诊断。
- [详细报告](reports/gate_0902_auto2_20260921/report.md)、
  [可复现脚本](reports/gate_0902_auto2_20260921/diagnose.py)、
  [机器证据及源哈希](reports/gate_0902_auto2_20260921/evidence.json)。
